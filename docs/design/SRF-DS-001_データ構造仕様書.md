...
### 3.3 関数提供型優先スコア（目標仕様・未実装）

関数提供型・モジュール型リポジトリを優先する改定後の評価は、特定分野の依存ライブラリ数ではなく、再利用可能な提供物を根拠にする。詳細な要件は [SRF-FEAT-003](../features/FEAT_function_oriented_repository_selection.md) および [ADR-001](../adr/ADR-001-関数提供型リポジトリを優先する.md) を参照する。

| 評価軸     | 主な根拠                                                      | 方針                                     |
| :--------- | :------------------------------------------------------------ | :--------------------------------------- |
| 再利用性   | 公開API、公開ヘッダ、import可能なパッケージ、責務別モジュール | 最優先の評価軸とする                     |
| 利用可能性 | ビルド・導入手順、利用例、設定可能な入出力                    | 他プロジェクトへの組込みやすさを評価する |
| 保守性     | テスト、CI、明確な構成、ライセンス                            | 品質根拠として評価する                   |
| 研究文脈   | 論文・DOI・学術キーワード                                     | 補助的な根拠として扱う                   |

ML・科学計算・Papers with Codeの属性は、検索・表示用に保持しても、再利用性の直接加点には使用しない。改定後の配点・閾値は、提供形態別およびシードカテゴリ別の通過率を計測したうえで決定する。
