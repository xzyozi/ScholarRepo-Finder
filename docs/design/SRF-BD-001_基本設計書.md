...
配点、評価軸の上限、ハードフィルター、掲載閾値、信頼度乗数は `config/scoring.json` で版管理し、評価器は検証済みの設定オブジェクトだけを受け取る。これにより、配点の見直しをコード変更から分離し、同一の設定ID・版・ハッシュで評価結果を再現する。具体的な用語と判断理由は [SRF-CX-001 用語集](./SRF-CX-001_用語集.md)、[ADR-001](../adr/ADR-001-関数提供型リポジトリを優先する.md)、および [SRF-SC-001 スコアリング設定仕様書](./SRF-SC-001_スコアリング設定仕様書.md) を正とする。
