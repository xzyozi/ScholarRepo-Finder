...
### 2.2 設定駆動の関数提供型候補選定フロー（目標仕様・未実装）

1. **設定検証**: 評価開始前に `config/scoring.json` を読み込み、スキーマ、未知キー、必須キー、数値範囲、評価軸の上限を検証する。検証失敗時は明示的なエラーで処理を停止し、配信データを更新しない。
...
7. **観測値の出力**: 提供形態別・シードカテゴリ別に、収集数、ハードフィルター通過数、閾値通過数、最終掲載数、評価軸別内訳、設定ID・版・ハッシュを出力する。
